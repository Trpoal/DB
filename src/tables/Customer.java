...
public class Customer {
    ...
    private ObservableList<Customer> info;

    public Customer(){}

    public Customer(int id, String surname, String name, String patronymic, Date dateOfBirth, long passport, String address, long phone, String email)
    {
        this.idCustomer = id;
        this.name = name;
    ...
    public LinkedList<TableColumn> listColumn() {
        ...
        TableColumn<Car, String> idCustomer = new TableColumn("ID");
    ...
    public ObservableList takeInfo(Connection con) {
        ...
        ArrayList<Customer> dir = new ArrayList<>();
        String prepSt = "select * from customer";
        try {
            pr = con.prepareStatement(prepSt);
            ResultSet rs = pr.executeQuery();
            while (rs.next()) {
                dir.add(new Customer(rs.getInt(1), rs.getString(2), rs.getString(3),
    ...
    public void addRowToDataBase(Connection con,  ObservableList info) {
        ...
        String query = "insert into customer (surname, name, patronymic," +
...
// get a connection and then in your try catch for executing your delete...
    ...
    public void editNote(Connection con, int indexObs, ObservableList info) {
        String SQL = "update customer set surname=?, name = ?, patronymic = ?, dateOfBirth = ?, passport = ?," +
                "address = ?, phone = ?, email =?  where idCustomer = ?";
        PreparedStatement preparedStmt = null;
        ...
        try {
            ...
            preparedStmt.setLong(7, phone);
            preparedStmt.setString(8, email);
            preparedStmt.setInt(9, idCustomer);
            preparedStmt.executeUpdate();
        } catch (SQLException e) {
            System.out.println("MISTAKE");
            e.printStackTrace();
        }
        Customer tmp = (Customer) info.get(indexObs);
    ...
    public ObservableList takeFIO(Connection con) {
        ...
        String prepSt = "select idcustomer, surname, name, patronymic from customer";
        try {
